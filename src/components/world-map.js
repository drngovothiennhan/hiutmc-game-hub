...
export function renderWorldMap(session, entitlement = null, authenticated = false, member = null) {
  const adminPracticeCard = isGameHubAdmin(member)
    ? '<article class="place-card yquan-admin-card" data-place-id="y-quan-admin-preview"><div class="place-top"><span class="place-emblem" aria-hidden="true">醫</span><span class="tag tag-live">Admin preview · riêng quyền</span></div><p class="place-area">PHÒNG THỰC HÀNH · BẢN XEM TRƯỚC</p><h3>Y Quán · Thực hành vấn chẩn</h3><p class="place-description">Thực hành Thập vấn với ca mô phỏng hư cấu. Quyền vào phòng được xác minh lại từ HIU TMC.</p><a class="place-action" href="/y-quan-practice/">Mở phòng thực hành <span aria-hidden="true">→</span></a></article>'
    : '';
  return `<section class="world-intro"><div class="world-copy"><p class="eyebrow">HIU TMC GAME HUB</p><h1>BẢN ĐỒ HỌC THUẬT</h1><p>Không gian tập trung các trò chơi học thuật Y học cổ truyền của HIU TMC. Gia Viên Dược Thảo đang chạy tại Study OS; khi hoàn thành đủ 9 ô, bạn tiếp tục chính hành trình này tại Game Hub sau khi hệ thống xác nhận hoàn thành đủ 9 ô.</p><div class="world-legend"><span><i class="legend-dot legend-dot-live"></i>Đang chạy tại Study OS</span><span><i class="legend-dot legend-dot-next"></i>Chuẩn bị theo giai đoạn</span></div></div><div class="world-seal" aria-hidden="true"><span>醫</span><small>HIU · TMC</small></div></section><section class="map-section" aria-label="Bản đồ học thuật"><div class="section-heading"><div><p class="eyebrow">BẢN ĐỒ HỌC THUẬT</p><h2>Chọn khu vực</h2></div><span class="map-caption">Các khu vực được mở theo từng giai đoạn</span></div><div class="place-grid">${worldMap.map(place => placeCard(place, session, entitlement, authenticated)).join('')}${adminPracticeCard}</div></section>`;
}
