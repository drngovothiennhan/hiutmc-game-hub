...
const sprite='/garden-decor-sprite.svg';
...
export default function HerbGardenGame({member}:{member:Member}){
  ...
  return <section className="herb-garden-page herb-garden-v3 garden-pro-v7">
    <header className="garden-hero garden-pro-hero">
      <div>
        <span className="garden-kicker"><Sparkles/> Gia Viên Dược Thảo · tiếp tục hành trình</span>
    ...
    {adminPreview&&<section className="garden-admin-preview panel" aria-labelledby="garden-admin-preview-title" data-testid="garden-admin-preview">
      ...
      <div className="garden-admin-stages">
        <article><span className="garden-expansion-kicker">Chặng 1 · Vườn khởi đầu</span><h3>Gieo trồng và chăm cây</h3><p>Chọn 3 ô đầu tiên, gieo hạt, chăm theo chu kỳ 72 giờ rồi thu hoạch. Các ô tiếp theo chỉ mở theo xác nhận tiến trình từ máy chủ.</p><div className="garden-admin-stage-grid" aria-label="Chọn một ô để mở bảng chăm sóc">{Array.from({length:9},(_,i)=>{const slot=i+1;const plot=plots.find(x=>x.slot_no===slot);const chosen=initialChoice.includes(slot);const status=plot&&plot.id?(plot.name||"Cây đang trồng")+", "+growth(plot,now)+"%":plot&&plot.unlocked?"ô trống":"đang khóa";return <button type="button" key={slot} disabled={busy||!plot||(initialComplete&&!plot.unlocked)} onClick={()=>focusLivePlot(slot)} className={"garden-admin-stage-plot "+(plot&&plot.unlocked?"is-open":"is-locked")+(plot&&plot.id?" has-plant":"")+(chosen?" is-selected":"")+(plot&&plot.status?" stage-"+plot.status:"")} aria-label={"Ô "+slot+", "+status+(chosen?", đã chọn":"")} title={"Ô "+slot+" · "+status}><span className={"garden-admin-mini-pot"+(plot&&plot.id?" has-plant":"")} aria-hidden="true"><i/></span><span className="garden-admin-mini-number">{slot}</span></button>})}</div><small>Chạm ô để mở đúng ô trong khu vườn 3×3 bên dưới. Trạng thái vẫn lấy từ máy chủ.</small></article>
    ...
    <div className="garden-v3-layout garden-pro-layout">
      <section id="garden-live-board" className={`garden-board-v3 garden-pro-board garden-scene-theme-${profile.theme}`} aria-label="Gia Viên 9 ô">
