...
import { createQuestionPlan, scoreAttempt } from './engine.js';
...
function categoryPanel() {
  ...
  return `<aside class="question-board"><div class="board-heading"><span class="iq-kicker">CHỌN NỘI DUNG HỎI</span><b>${asked.length}/15 câu</b></div><div class="progress-track"><span style="width:${Math.round(asked.length/15*100)}%"></span></div><div class="question-groups">${grouped}</div><button class="iq-secondary finish-interview" data-action="finish-interview" ${asked.length<15?'disabled':''}>Hoàn tất hỏi bệnh</button></aside>`;
}
function pulse() {
  ...
  return `<section class="room-panel room-entry" tabindex="-1"><div class="scene-title"><div><span class="iq-kicker">PHÒNG BẮT MẠCH · VẤN CHẨN</span><h2>Hãy hỏi để hiểu bệnh cảnh</h2></div><span class="turn-counter">${current}/15</span></div><div class="consultation-layout"><div class="conversation"><div class="stage characters">${doctorCard()}${patientCard(last?.expression || 'neutral')}</div>${questionMarkup}${findingMarkup}<div class="transcript-list"><strong>Diễn tiến hội thoại</strong>${transcript.slice(-5).map(line=>`<p><b>${esc(line.speaker)}:</b> ${esc(line.text)}</p>`).join('')}</div></div>${categoryPanel()}</div></section>`;
}
...
function pharmacy() {
  ...
  return `<section class="room-panel room-entry" tabindex="-1"><div class="scene-title"><div><span class="iq-kicker">PHÒNG CHẾ DƯỢC · TỔNG KẾT HỌC TẬP</span><h2>Phản hồi ca bệnh</h2></div><span class="score-seal">${accuracy}<small>/100</small></span></div><div class="result-grid"><div class="result-paper"><span class="iq-kicker">NHẬN ĐỊNH CA</span><h3>${esc(caseFile.pattern)}</h3><p>Đáp án của ca mô phỏng: ${esc(caseFile.b8c.join(' · '))}.</p><p><b>Gợi ý ghi nhận thêm:</b> ${missed.length?missed.map(esc).join(' · '):'Bạn đã chọn đủ dữ kiện trọng tâm của rubric mẫu.'}</p><p><b>Nguyên tắc:</b> ${esc(caseFile.principles[0])}. Đây là mục tiêu học tập, không phải hướng dẫn điều trị.</p></div><div class="result-feedback"><span class="iq-kicker">TIẾN ĐỘ</span><h3>${performance}</h3><p>Điểm phản ánh độ phủ câu hỏi, dữ kiện ghi nhận và lựa chọn trong bài tập; không đánh giá năng lực khám chữa bệnh.</p><div class="result-actions"><button class="iq-primary" data-action="new-case">Luyện ca khác</button><a class="iq-secondary link-button" href="/y-quan-live/">Về Y Quán</a></div></div></div><p class="disclaimer">Tình huống tổng hợp dùng cho đào tạo. Không dùng để tự chẩn đoán hoặc tự điều trị. Nội dung cần giảng viên YHCT thẩm định trước khi sử dụng làm học liệu chính thức.</p></section>`;
}
function render() {
  if (!signedIn()) {
  ...
  root.innerHTML = `${header()}${view}<footer class="iq-footer">HIU Y Quán · Nội dung đào tạo mô phỏng · Không lưu dữ liệu ca bệnh</footer>`;
}
root.addEventListener('click', event => {
  ...
  if (button.dataset.question) {
    ...
    asked.push(question);
    const answer=caseFile.answers[question.id];
