...
let session=null,data=null,clinics=[],doctorVisits=[],patientVisits=[],leaderboard=[],page='doctor',busy=false,message='',stars={},activeChat=null,chatMessages=[],chatNotice='';
...
async function refreshChat(quiet=false){if(!activeChat)return;try{chatMessages=await rpc('y_quan_visit_messages_v1',{p_visit_id:activeChat.visitId},'patient');chatNotice='';const box=ROOT.querySelector('#chat-messages'),status=ROOT.querySelector('#chat-status');if(box){box.innerHTML=chatMessagesHTML();box.scrollTop=box.scrollHeight}if(status)status.textContent=''}catch(e){if(!quiet){chatNotice=e.message;const status=ROOT.querySelector('#chat-status');if(status)status.textContent=chatNotice}}}
async function openChat(visit){activeChat={visitId:visit.visit_id,otherName:page==='doctor'?visit.patient_name:visit.doctor_name};chatMessages=[];chatNotice='';await refreshChat();render();const box=ROOT.querySelector('#chat-messages');if(box)box.scrollTop=box.scrollHeight}
