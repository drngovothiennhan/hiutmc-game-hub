...
test('Garden RPC reports timeout as an ambiguous write and does not retry it', async t => {
  installStorage(t, { accessToken: 'fixture-access-token', refreshToken: 'fixture-refresh-token', expiresAt: Date.now() + 3600000 });
  const originalFetch = globalThis.fetch;
  let rpcCalls = 0;
  ...
  const result = await gardenSupabase.rpc('herb_garden_plant_v3', { p_slot_no: 1 });

  assert.equal(rpcCalls, 1);
  assert.equal(result.data, null);
  assert.equal(result.error.code, 'GARDEN_RPC_TRANSPORT');
});
